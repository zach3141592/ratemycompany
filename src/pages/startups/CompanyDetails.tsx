...
import {
  fetchStartupLeaderboardEntry,
  fetchStartupEloHistory,
  fetchStartupReviews,
  submitStartupReview,
  toggleStartupReviewReaction,
  StartupReview,
} from "@/data/startups";
...
const CompanyDetails = () => {
  ...
  const {
    ...
    queryKey: ["startup", companyId],
    queryFn: () => fetchStartupLeaderboardEntry(companyId),
  ...
  const {
    ...
    queryKey: ["startupEloHistory", companyId],
    queryFn: () => fetchStartupEloHistory(companyId),
    enabled: Boolean(companyId),
  });
  ...
  const {
    ...
    queryKey: ["startupReviews", companyId],
    queryFn: () => fetchStartupReviews(companyId),
    enabled: Boolean(companyId),
  });
  ...
  const submitReviewMutation = useMutation({
    mutationFn: async () => {
      ...
      await submitStartupReview(companyId, payload, user?.id ?? null);
    },
    onSuccess: async () => {
      setShowReviewForm(false);
      setReviewForm({
      ...
      setHasLocalReviewSubmission(true);
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["startupReviews", companyId] }),
        queryClient.invalidateQueries({ queryKey: ["startup", companyId] }),
  ...
  const toggleReactionMutation = useMutation({
    mutationFn: async (review: StartupReview) => {
      if (!user) {
        throw new Error("You must be signed in to like a review.");
      }
      await toggleStartupReviewReaction(review.id, user.id);
    },
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["startupReviews", companyId] });
    },
    onError: (error: unknown) => {
  ...
  const handleToggleReaction = (review: StartupReview) => {
    if (!user) {
